...
export default function ProductManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isEditMode 
      ? `http://${process.env.NEXT_PUBLIC_API_URL}/api/products/${currentProductId}`
  ...
  const handleDelete = async (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const token = localStorage.getItem('token');
        try {
          const res = await fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/products/${id}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`
            }
                ...
                <tr key={product.id} className="border-b border-gray-50 last:border-none hover:bg-slate-50">
                  ...
                  <td className="py-4 align-middle">
                    <div style={{ width: '50px', height: '50px', borderRadius: '8px', overflow: 'hidden', backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #e2e8f0' }}>
                      <img 
                        src={product.image ? `http://${process.env.NEXT_PUBLIC_API_URL}/uploads/products/${product.image}` : 'https://placehold.co/100?text=No+Img'} 
                        alt={product.name}
