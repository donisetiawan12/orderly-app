...
export default function Hero({
  ...
  const handleUpdateNotes = async (orderId: number) => {
    ...
    try {
      const res = await fetch(`/api/api/orders/${orderId}/notes`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSendReview = async (orderId: number) => {
    ...
    try {
      const res = await fetch('/api/api/orders/review', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
                          ...
                          <div style={{ fontSize: '12px', color: '#1f2937' }}>
                            ...
                            {order.seller_qris_image && (
                              <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                ...
                                <img src={`/api/uploads/payments/${order.seller_qris_image}`} alt="QRIS Seller" style={{ width: '160px', height: '160px', objectFit: 'contain', borderRadius: '14px', backgroundColor: '#fff', padding: '6px', border: '1px solid #e5e7eb' }} onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/150?text=QRIS+Error'; }} />
                                <button type="button" onClick={() => handleDownloadQris(`/api/uploads/payments/${order.seller_qris_image}`, order.id)} style={{ marginTop: '8px', padding: '6px 14px', backgroundColor: '#16a34a', color: '#ffffff', border: 'none', borderRadius: '8px', fontSize: '11px', fontWeight: '700', cursor: 'pointer' }}>📥 Unduh Gambar QRIS</button>
                              </div>
                            )}
                          </div>
                        ) : (
